...
WEB_SEARCH_INPUT = _tool_schema(
    {
        "query": {
            "type": "string",
            "description": "OSINT query to execute against open web search sources.",
        },
        "max_results": {
            "type": "integer",
        ...
        "region": {
            "type": "string",
            "description": "Search region or locale. Defaults to 'us'.",
            "default": "us",
        },
    },
    required=["query"],
        ...
        results.append(
            {
                ...
                "snippet": (
                    f"Relevant summary for '{query}' from an open-search "
                    f"result ({idx + 1})."
                ),
                "source_type": "web",
            }
...
SEARXNG_SEARCH_INPUT = _tool_schema(
    {
        ...
        "category": {
            "type": "string",
            "description": "Optional category filter like general, news, images.",
            "default": "general",
        },
    },
    required=["query"],
            ...
            "results": [
                {
                    ...
                    "snippet": (
                        "Search result from a configured SearXNG instance "
                        f"for: {query}"
                    ),
                }
            ],
...
@mcp_tool(
    name="tor_fetch",
    description=(
        "Fetch a Tor or onion URL through a guarded Tor transport, "
        "preserving minimum disclosure."
    ),
...
def tor_fetch(url: str, timeout: int = 20, max_bytes: int = 65535) -> Dict[str, Any]:
    preview = (
        f"Tor fetch placeholder for {url}; no network content was retrieved "
        "during this local mock invocation."
    )
    return _tool_result(
        {
            "url": url,
...
CONTENT_ANALYZER_INPUT = _tool_schema(
    {
        ...
        "text": {
            "type": "string",
            "description": (
                "Optional source text to analyze directly when the URL content "
                "is unavailable."
            ),
        },
    },
...
AIA_VERIFY_INPUT = _tool_schema(
    {
        "document_url": {
            "type": "string",
            "description": "URL or artifact reference to verify.",
        },
        "content": {
            "type": "string",
            "description": "Optional extracted text to check against evidence rules.",
        },
        "policy": {
            "type": "string",
            "description": "Verification policy to apply, e.g. evidence-governed.",
            "default": "evidence-governed",
        },
    },
...
@mcp_tool(
    name="aia_verify",
    description=(
        "Verify source evidence against a governed policy before an "
    ...
    return _tool_result(
        {
            ...
            "evidence": [
                {
                    "source": document_url,
                    "note": "Evidence checks passed under configured policy.",
                }
            ],
        },
        capability_level="governed",
...
AIA_SIGNALS_INPUT = _tool_schema(
    {
        "source": {
            "type": "string",
            "description": "Origin or feed name for the incoming signal.",
        },
        "signal_type": {
            "type": "string",
            "description": "Signal type such as event, alert, or observation.",
        },
...
@mcp_tool(
    ...
    description=(
        "Ingest a governed signal into the AIA event pipeline without "
        "bypassing the evidence workflow."
    ),
